...
from utils.dataset_evaluation import TUBerlinDataset
...
ckpt_path = './checkpoints/saved_models/TU-Berlin/epoch=56-top10=0.00.ckpt'
...
tuberlin_zero_ext = TUBerlinDataset(split='zero', version='ImageResized_ready', zero_version='zeroshot',
                                    transform=transformations, aug=False)

zero_loader_ext = DataLoader(dataset=tuberlin_zero_ext, batch_size=80, shuffle=False, num_workers=0)

tuberlin_zero = TUBerlinDataset(split='zero', zero_version='zeroshot', transform=transformations, aug=False)
zero_loader = DataLoader(dataset=tuberlin_zero, batch_size=80, shuffle=False, num_workers=0)
...
for idx, sk_feat in enumerate(predicted_features_query):
    distance = -1 * distance_fn_(sk_feat.unsqueeze(0), predicted_features_gallery)
...
for fi in range(predicted_features_query.shape[0]):
    mapi = eval_AP_inner(gt_labels_query[fi], scores_1[fi], gt_labels_gallery, top=None)
...
for fi in range(predicted_features_query.shape[0]):
    prec = eval_precision(gt_labels_query[fi], scores_1[fi], gt_labels_gallery, top=100)
