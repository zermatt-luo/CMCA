...
from utils.dataset_train import QuickDraw
from utils.options import opts

if __name__ == '__main__':
    dataset_transforms = QuickDraw.data_transform(opts)

    train_dataset = QuickDraw(opts, dataset_transforms, mode='train', return_orig=False)
    val_dataset = QuickDraw(opts, dataset_transforms, mode='val', used_cat=train_dataset.all_categories, return_orig=False)
    ...
    logger = TensorBoardLogger('./checkpoints/tb_logs', name='QuickDraw/')
    dirpath = './checkpoints/saved_models/%s' % 'QuickDraw/'
    print(dirpath)
    ...
    ckpt_path = os.path.join('./checkpoints/saved_models', 'QuickDraw/', 'last.ckpt')
